...
- `/se cast <skillId> [player]` — принудительно выполнить каст навыка.
  - `<skillId>` — `id` навыка из YAML (или имя файла, если `id` не задан).
  - `[player]` — опционально: ник игрока, на которого выполнить каст (по умолчанию — вы сами).
